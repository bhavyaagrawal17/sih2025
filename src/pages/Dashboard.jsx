...
function Dashboard() {
  ...
  const filtered = useMemo(
    () => recommendations.filter(r => filter === 'All' || r.location === filter),
    [filter]
  )
  ...
  return (
    <div className="container mx-auto py-8">
      {/* Greeting & Edit Profile Button */}
      <div className="card p-6 bg-gradient-to-r from-blue-50 to-emerald-50 mb-6">
        ...
        <div className="mt-4">
          <button className="btn-primary" onClick={() => setIsEditing(v => !v)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="card p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(profile.skills || []).slice(0, 8).map(s => (
                <span key={s} className="px-2 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200 text-xs">
                  {s}
                </span>
              ))}
          ...
          <div className="flex items-center justify-between">
            ...
            >
              {['All', 'Remote', 'Delhi', 'Bengaluru'].map(x => (
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {filtered.map(item => (
              <div key={item.id} className="card p-5 flex flex-col gap-3">
                ...
                <button className="mt-auto inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 py-2 font-semibold hover:from-orange-600 hover:to-orange-700 transition">
                  Apply
                </button>
              </div>
            ))}
          ...
          <div className="card p-5">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              {['UI/UX Designer', 'ML Engineer', 'Product Manager', 'QA Tester'].map(x => (
                <div key={x} className="rounded-lg border border-orange-200 bg-orange-50 p-3 text-sm">
                  {x}
                </div>
              ))}
            </div>
